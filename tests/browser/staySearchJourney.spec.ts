...
const searchUrl = (destination: string, arrival: string, departure: string, guests = 2) =>
  ORIGIN + '/en/stays/availability-booking?destination=' + destination + '&arrival=' + arrival +
...
test('finder explains a broken stay range and resets for a new arrival', async ({ page }) => {
  ...
  await page.goto(ORIGIN + '/en/stays/availability-booking?destination=frigiliana');
...
test('result handoff preserves provider-neutral search state and primes the stay quote', async ({ page }) => {
  ...
  await expect(maha.locator('[data-am-stay-booking-link]')).toHaveAttribute(
    'href',
    new RegExp('/en/stays/frigiliana/la-amara-maha\\?arrival=' + arrival + '&departure=' + departure + '&guests=2')
  );
  const beforeHandoff = requests.length;
  await maha.locator('[data-am-stay-booking-link]').click();
  await expect(page).toHaveURL(new RegExp('/en/stays/frigiliana/la-amara-maha\\?arrival=' + arrival + '&departure=' + departure + '&guests=2'));
...
test('all five utility routes are noindex and localized without starting a search', async ({ page }) => {
  const routes = [
    ['/en/stays/availability-booking', 'Destination'], ['/de/unterkuenfte/verfuegbarkeit-buchung', 'Reiseziel'],
    ['/alojamientos/disponibilidad-reserva', 'Destino'], ['/nl/verblijven/beschikbaarheid-boeking', 'Bestemming'], ['/sv/boenden/tillganglighet-bokning', 'Resmål']
  ] as const;
