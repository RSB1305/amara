...
export const frigilianaGeographyContent = {
  ...
  closing: {
    ...
    body: l("Casa AMARA sits on Calle Chorruelo, where Calle Real continues through the historic village. That places the stay on this mental map; the Streets & Stairs guide explains what the position means for the routes you repeat each day.", "Casa AMARA liegt an der Calle Chorruelo, dort setzt sich die Calle Real durch den historischen Ort fort. Damit ist die Unterkunft auf dieser mentalen Karte verortet; der Guide zu Gassen und Treppen erklärt, was die Lage für eure täglichen Wege bedeutet.", "Casa AMARA está en la calle Chorruelo, donde continúa la calle Real por el pueblo histórico. Así queda situada en este mapa mental; la guía de calles y escaleras explica qué significa la ubicación para los recorridos que repetís cada día.", "Casa AMARA ligt aan Calle Chorruelo, waar Calle Real door de historische kern verderloopt. Zo krijgt het verblijf een plek op deze mentale kaart; de gids over straten en trappen legt uit wat de ligging betekent voor jullie dagelijkse routes.", "Casa AMARA ligger på Calle Chorruelo, där Calle Real fortsätter genom den historiska byn. Det placerar boendet på den här mentala kartan; guiden om gränder och trappor förklarar vad läget betyder för era dagliga vägar."),
