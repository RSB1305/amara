...
export interface TarifaKitesurfWindContent {
  ...
  thermal: TarifaKitesurfChapterHead & {
    ...
    /** What the tidal current does to the wind a rider feels; the tide itself is in the beach chapter. */
    tides: { title: LocalizedText; intro: LocalizedText; cards: Array<{ id: string; title: LocalizedText; text: LocalizedText }>; note: LocalizedText };
  };
...
export const tarifaKitesurfWindContent: TarifaKitesurfWindContent = {
  ...
  thermal: {
    ...
    tides: {
      ...
      note: l('In the Windguru table the tide is the curve in the **bottom row**; on the beach you see it at the waterline. At high tide Los Lances gets narrow, see “Where to launch”.', 'In der Windguru-Tabelle steht die Tide als Kurve in der **untersten Zeile**; am Strand seht ihr sie an der Wasserlinie. Bei Flut wird Los Lances schmal, das steht unter „Wo raus“.', 'En la tabla de Windguru la marea es la curva de la **última fila**; en la playa la veis en la línea del agua. Con marea alta Los Lances se estrecha, está en «Dónde salir».', 'In de Windguru-tabel is het tij de curve in de **onderste rij**; op het strand zien jullie het aan de waterlijn. Bij vloed wordt Los Lances smal, dat staat onder “Waar het water op”.', 'I Windguru-tabellen är tidvattnet kurvan på **nedersta raden**; på stranden ser ni det vid vattenlinjen. Vid högvatten blir Los Lances smal, det står under ”Var man går ut”.')
    }
  },
  beach: {
    ...
    tides: {
      ...
      note: l('And the tide works out on the water too: when the tidal current runs off downwind, **everyone suddenly loses ground upwind** — Mark’s pointer from further up.', 'Und die Tide arbeitet auch draußen: Zieht die Gezeitenströmung in Windrichtung ab, **verliert plötzlich jeder Höhe** — Marks Hinweis von weiter oben.', 'Y la marea también trabaja fuera: cuando la corriente de marea tira en la dirección del viento, **todos pierden altura de repente**, la pista de Mark de más arriba.', 'En het tij werkt ook buiten: trekt de getijstroom met de wind mee weg, dan **verliest iedereen ineens hoogte** — Marks aanwijzing van hierboven.', 'Och tidvattnet jobbar även därute: när tidvattenströmmen drar iväg i vindriktningen **tappar alla plötsligt höjd** — Marks tips längre upp.')
    }
  },
  emergency: {
