...
| Actions und Links | `AmaraActionLink` und `AmaraActionButton` kapseln die bestehenden Primary-/Secondary- sowie CTA-, Inline-, Guide- und Card-Treatments. Je ein Trust-, Stay-, Location- und Experience-Referenzverbraucher ist ohne visuelle Änderung migriert; der Styleguide rendert die Produktionskomponenten. | Restliche direkte Klassenverwendungen nach UI-Job clustern und kontrolliert migrieren; Inverse-, Disabled- und Loading-Anwendungen im nächsten Actions-Schnitt vollständig abdecken. |
