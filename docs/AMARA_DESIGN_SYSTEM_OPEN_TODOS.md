...
| DS-03 | **IN ARBEIT** | Lebender AMARA-Styleguide | Der Typografie-Restbestand ist vollständig geschlossen: Der Ratchet steht bei 0. Der Styleguide rendert für Typografie, Actions, Links, Chips, Status, Hero-Familien sowie Cards/Inhaltsmodule ausschließlich Produktionsowner; weitere Modulcluster werden bei ihrer Freigabe ergänzt. |
...
| Cards und Inhaltsmodule | Es gibt keine Universal-Card. `moduleContract.ts` klassifiziert Feature-Grid und praktischen Callout als kanonische Jobs; Guide-Link-Grid, Stay-Auswahl, Evidence, Vergleich, Buchungsentscheidung und Editorial Statement bleiben benannte Familienmodule. Sechs wiederkehrende Location-Renderer verwenden den neuen `EditorialCallout`; der Styleguide rendert nur Produktionsowner. | Abgeschlossen für den aktiven Cluster. Verbleibende lokale Flächenrezepte sind kontrolliertes Legacy und werden bei materieller Überarbeitung nach Job zugeordnet; neue generische Card-Rezepte sind nicht zulässig. |
| Interne Designansichten | Der Living Styleguide zeigt in den freigegebenen Clustern ausschließlich Produktionsowner. Component Library und Design Lab zeigen weiterhin nicht durchgehend denselben Status. | Component Library als Bestandsanzeige klar vom verbindlichen Styleguide und experimentellen Design Lab trennen. |
