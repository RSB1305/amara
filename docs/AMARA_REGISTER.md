...
last_modified: 2026-08-23T11:40:00+02:00
...
| DR-CONTENT-STRUCT-002 | The five locales of a content module must carry the same **semantic units**: a key present in one locale exists in all of them, and a list of units — FAQ entries, cards, themes, sections, CTA blocks — holds the same entries, in the same order, under the same stable identifiers. **Prose segmentation is explicitly not covered.** A locale may split or join paragraphs and list items of running text as its language requires; that is TR-CONTENT-002 working as intended, not a gap, and `LocalizedTextList` is the type that expresses it. Parity is enforced by contract test rather than by review. Gaps are recorded there with what is missing; a recorded gap may only be closed by authored copy in the missing languages, and a gap that no longer exists must be removed from the record. | ACTIVE |
