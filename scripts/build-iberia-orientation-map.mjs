 ...
 * (src/components/location/FrigilianaOrientationMap.astro).
 *
 ...
 * plus projected city positions and the road-shaped round-trip line.
 *
...
// Round trip from the AMARA base, following the actual road corridors:
// Frigiliana → Nerja → A-7 → Málaga → A-357/A-367 → Ronda → A-374/A-375 → Sevilla → A-4 → Córdoba
// → N-432 → Granada → A-44 → Costa Tropical → A-7 → Nerja → Frigiliana.
const ROUTE = [
  [-3.8954, 36.7892], // Frigiliana
  [-3.8765, 36.7451], // Nerja
  [-3.95, 36.735], [-4.09, 36.74], [-4.28, 36.72],
  [-4.4214, 36.7213], // Málaga
  [-4.63, 36.715], [-4.72, 36.8], [-4.85, 36.88], [-5.02, 36.85],
  [-5.166, 36.742], // Ronda
  [-5.41, 36.88], [-5.62, 37.05], [-5.78, 37.18],
  [-5.9845, 37.3891], // Sevilla
  [-5.64, 37.47], [-5.08, 37.54], [-4.9, 37.72],
  [-4.7794, 37.8882], // Córdoba
  [-4.55, 37.75], [-4.32, 37.62], [-3.93, 37.46], [-3.7, 37.3],
  [-3.5986, 37.1773], // Granada
  [-3.62, 37.02], [-3.55, 36.88], [-3.6, 36.76], [-3.69, 36.735], [-3.74, 36.735], [-3.83, 36.755],
  [-3.8765, 36.7451], // Nerja
  [-3.8954, 36.7892] // Frigiliana
];
...
const route = smoothPath(ROUTE.map(([lon, lat]) => proj(lon, lat)));
