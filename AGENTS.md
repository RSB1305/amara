...
- Public editorial heroes use a concise H1 that occupies no more than two visible lines at the reference mobile and desktop viewports. The H1 names the page subject; a medium subtitle carries the positioning or promise, and the explanatory/direct-answer text follows beneath it. Curate the copy in every locale; never enforce the limit by clipping or truncating the heading.
...
- Equal-surface page sections use the single centrally owned inset separator: the line aligns with the page content gutter and never reaches the outer edge of the white page surface. `AmaraSection` applies this geometry automatically to plain default/compact sections; page-local divider switches and full-width outer rules are prohibited. Editorial boxes, display bands, CTA panels, prose, pull quotes, bylines and provenance blocks carry no decorative top/bottom edge rules. Structural borders remain permitted where lists, tables, controls or media frames require them for comprehension or interaction. `check:section-boundary-policy` (prebuild and `npm run check`) fails the build for any page-gutter band (`px-6 md:px-12`) that carries `border-t`, `border-b` or `border-y`.
- Public routes are declared once in `src/lib/publicRouteManifest.mjs` (language-neutral key, parent, page family, native path per locale) and rendered by the two catch-all pages through `src/pages/_routes/PublicRoutePage.astro`; do not add explicit route files. Use existing `linkRegistry.ts` and `routeOwnership.ts` contracts where applicable. Do not invent silent fallbacks, aliases or token behavior.
...
New and materially revised public editorial copy must pass the **people-first host-language gate** (`TR-CONTENT-008`). Write as a relaxed, knowledgeable host would speak to guests: answer the concrete question, use plain verbs and keep only facts, consequences, truthful scenes, limitations or useful actions. Public editorial copy states directly what AMARA, a stay, a place or an experience offers; never define its value through denial, rejection or a contrast with what it is not. Phrases such as `not a ranking`, `not a generic list`, `not only X` and their localized equivalents fail the gate and are replaced by the concrete positive offer, fact, detail or scene. Repeated decision rhetoric (`choose`, `plan`, `compare`), guide or hub metasprache, paragraphs that merely restate a topic, and card series forced into the same translated sentence template are failures. Use an imperative only for a real guest action. A large section or presentation stage requires a correspondingly important content idea; otherwise combine or remove it. Read the rendered page aloud and ask whether the host would naturally say it. If not, rewrite or delete it.

**House style (operational reading of `TR-CONTENT-008`, binding since the 2026-09-08 rework of all public pages).** The host does the work, the guest decides: state what is, name where and when, and leave the choice to the reader. Concretely: one reservation per page, at the end, never a disclaimer per paragraph; a check request only with a place or a time (“the signs at the car park”, “the buoys in June”), at most two per page; no sentence about what something is not; no “honestly” or its equivalents, except the approved “Ehrlich dazu / Ehrlich bleibt” verdict pattern on the where-to-stay pages; scene before category (“Calahonda before ten, when the light falls on the white houses” before “central cove”); answers instead of tasks (“the car park is 400 m away” instead of “compare parking options”); every fact once per page; every page ends at AMARA (the stay, the terrace, the walk home); editorial process stays behind the stage — sources are a link list under “Where the details come from”, the byline carries one line “As of <month year>”, and no page describes itself as a guide, a service or a research result. Numbers only where they change a decision, as ranges where the value varies by person or season. Translations carry this tone natively; they are not word-for-word renderings of the German. Browser specs pin the wording of the guest-access intro (DE and EN) and the module order of the arrival guides; a text change there updates the spec in the same commit.

Every public editorial page speaks from AMARA's collective first-person host perspective: we/us/our and the natural equivalents in EN, DE, ES, NL and SV. Never narrate AMARA, its hosts or Robert from outside in the third person; named people remain visible through the byline and author metadata. Every guest address is plural: German uses ihr/euch/euer consistently, with the corresponding native plural forms in the other locales. Natural pronoun omission may preserve fluent language, but it never changes the collective-host-to-plural-guests perspective. This voice must not turn researched or third-party evidence into an invented first-party observation.
...
The canonical header availability action uses the shared calendar-check icon in its white inverse circular treatment at every viewport and keeps a native localized accessible label in EN, DE, ES, NL and SV; the mobile menu retains the visible localized label.
...
- Every Codex Cloud implementation must hand off through its dedicated remote branch and an open pull request. A commit-only reference, patch file, archive, Base64 block, clipboard transfer or other manual artifact transfer is not an accepted handoff. Analysis-only Cloud tasks with no repository changes are exempt. Cloud agents do not merge directly to `main` unless the operator explicitly assigns them the release-controller role.
