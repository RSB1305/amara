...
New and materially revised public editorial copy must pass the **people-first host-language gate** (`TR-CONTENT-008`). Write as a relaxed, knowledgeable host would speak to guests: answer the concrete question, use plain verbs and keep only facts, consequences, truthful scenes, limitations or useful actions. Repeated decision rhetoric (`choose`, `plan`, `compare`), guide or hub metasprache, and paragraphs that merely restate a topic are failures. Use an imperative only for a real guest action. A large section or presentation stage requires a correspondingly important content idea; otherwise combine or remove it. Read the rendered page aloud and ask whether the host would naturally say it. If not, rewrite or delete it.
